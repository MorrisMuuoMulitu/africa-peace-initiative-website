...
const FloatingImpactCards = () => {
  ...
  return <div ref={ref} className="relative h-full min-h-[400px]">
      {/* Enhanced Desktop 3D Cards */}
      <div className="hidden lg:block relative h-full">
        {impactCards.map((card, index) => {
        const Icon = card.icon;
        return <div key={index} className={`absolute transition-all duration-1000 transform-3d hover-lift ${inView ? 'translate-x-0 opacity-100' : 'translate-x-20 opacity-0'}`} style={{
          top: `${index * 25 + 10}%`,
      ...
      <div className="lg:hidden">
        <div className="flex gap-6 overflow-x-auto snap-x snap-mandatory pb-6 px-4 scrollbar-hide">
          {impactCards.map((card, index) => {
          const Icon = card.icon;
          return <div key={index} className="flex-none w-72 snap-center">
